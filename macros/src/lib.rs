/*!
Procedural macros used in lumi, like [`macro@command`]
*/
...
/**
This macro transforms plain functions into lumi bot commands.
...
`#[lumi::command]` accepts a number of arguments to configure the command:
...
- `ephemeral`: Make bot responses ephemeral if possible (slash only)
    - Only lumi's functions, like `lumi::send_reply`, respect this preference
...
Internally, this attribute macro generates a function with a single `lumi::Command`
...
into something like
```rust
fn my_command() -> lumi::Command<Data, Error> {
