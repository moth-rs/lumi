...
pub fn command(
    ...
    fn permissions_to_tokens(
        ...
        match perms {
            Some(perms) => {
                ...
                syn::parse_quote! { #(lumi::serenity_prelude::Permissions::#perms)|* }
            }
            None => syn::parse_quote! { lumi::serenity_prelude::Permissions::empty() },
        }
    }
    ...
    let install_context = if let Some(contexts) = &args.install_context {
        ...
        syn::parse_quote! { Some(vec![ #(lumi::serenity_prelude::InstallationContext::#contexts),* ]) }
    } else {
    ...
    let interaction_context = if let Some(contexts) = &args.interaction_context {
        let contexts = contexts.iter();
        syn::parse_quote! { Some(vec![ #(lumi::serenity_prelude::InteractionContext::#contexts),* ]) }
    } else {
        syn::parse_quote! { None }
    };
    ...
    Ok(quote::quote! {
        ...
        > {
            ...
            ::lumi::Command {
                ...
                cooldowns: std::sync::Mutex::new(::lumi::Cooldowns::new()),
...
fn generate_cooldown_config(args: &CommandArgs) -> proc_macro2::TokenStream {
    ...
    quote::quote!(
        std::sync::RwLock::new(::lumi::CooldownConfig {
