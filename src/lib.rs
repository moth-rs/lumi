...
/*!
lumi is an opinionated Discord bot framework with a few distinctive features:
...
features of lumi: `cargo run --example=feature_showcase`

# Introduction to serenity

Serenity is the Discord API wrapper library lumi is built on top of. Using lumi automatically
...
is [MESSAGE_CONTENT](serenity::GatewayIntents::MESSAGE_CONTENT) which is required for lumi prefix
commands.
...
(lumi provides [`builtins::register_application_commands_buttons`] as a starting point for that), or you
...
## Create commands
Every command is represented by a function annotated with [`#[lumi::command]`](command):
...
#[lumi::command(prefix_command, /* add more optional command settings here, like slash_command */)]
...
See [`#[lumi::command]`](command) for detailed information.

### Subcommands
Commands in lumi have a tree structure. Every commands refers to a list of subcommands, which you
can easily set using the [`command`] macro like so:

```rust
# type Error = Box<dyn std::error::Error + Send + Sync>;
# type Context<'a> = lumi::Context<'a, (), Error>;
#[lumi::command(prefix_command, slash_command, subcommands("child1", "child2"))]
...
```rust
use lumi::{StrArg, serenity_prelude as serenity};
type Data = ();
type Error = Box<dyn std::error::Error + Send + Sync>;
type Context<'a> = lumi::Context<'a, Data, Error>;

/// A test command for lumi
#[lumi::command(
    prefix_command,
    track_edits,
    hide_in_help,
...
async fn my_huge_ass_command(
    ...
    #[description = "Sit"] code_block: lumi::CodeBlock,
...
async fn error_handler(error: lumi::FrameworkError<'_, Data, Error>) {
...
# type Error = Box<dyn std::error::Error + Send + Sync>;
# type Context<'a> = lumi::Context<'a, (), Error>;
# async fn my_error_function(_: lumi::FrameworkError<'_, (), Error>) {}
# #[lumi::command(prefix_command)] async fn command1(_ctx: Context<'_>) -> Result<(), Error> { Ok(()) }
# #[lumi::command(prefix_command)] async fn command2(_ctx: Context<'_>) -> Result<(), Error> { Ok(()) }
# #[lumi::command(prefix_command)] async fn command3(_ctx: Context<'_>) -> Result<(), Error> { Ok(()) }
use lumi::serenity_prelude as serenity;

# async {
// Use `Framework::builder()` to create a framework builder and supply basic data to the framework:

let framework = lumi::Framework::builder()
    ...
    .options(lumi::FrameworkOptions {
        on_error: |err| Box::pin(my_error_function(err)),
        prefix_options: lumi::PrefixFrameworkOptions {
            prefix: Some("~".into()),
            edit_tracker: Some(Arc::new(lumi::EditTracker::for_timespan(std::time::Duration::from_secs(3600)))),
...
commands need to be _registered_ to Discord. lumi provides several ways to do it, with varying
...
# struct UserData;
# struct ErrorType;
type Context<'a> = lumi::Context<'a, UserData, ErrorType>;
```

## Serenity prelude
When you're too lazy to import serenity items from their full path which can be quite lengthy at
times, you can use `lumi::serenity_prelude`: a module which reexports almost all items from
serenity.
...
pub mod track_edits;
pub mod macros {
    //! Procedural macros used in lumi, like [`command`]
    #[doc(inline)]
    pub use lumi_macros::*;
}
